...
class Flash
{
    ...
    {
        $values = \Dsc\Lib\ArrayHelper::only($values, $keys) + array_fill_keys($keys, null);
        $this->store($values);
    ...
    public function except( array $keys, array $values )
    {
        ...
        foreach ($keys as $key) 
        {
            \Dsc\Lib\ArrayHelper::forget($result, $key);
        }
        $this->store($result);
    ...
    {
        $array = $this->get('old');
        $value = \Dsc\Lib\ArrayHelper::get( $array, $key, $default );

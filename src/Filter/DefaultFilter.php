<?php 
namespace Dsc\Lib\Filter;

/**
 * Taken from https://github.com/FrozenNode/XssInput
 * And adapted for use with Phalcon 
 *
 */
class DefaultFilter 
{
    public function filter($source) 
    {
        if (is_array($source))
        {
            foreach ($source as $key => $value)
            {
                if (is_string($value))
                {
                    $source[$key] = $this->xss_clean($value);
                }
            }
